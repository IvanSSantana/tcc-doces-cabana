...
- [x] **T019** — `DocesCabana.MVC/Helpers/DbInitializer.cs`: `ImagensDeExemplo` troca os seis links de pré-visualização do Drive pelos seis endereços públicos do bucket `images`/`public` (RF-10). Comentado que são endereços públicos e por isso versionados, como os anteriores eram.
- [x] **T020** — Apagada a base local, subida a aplicação **sem credencial configurada**: a semeadura completa sem erro (RF-11, CA-11) e as cem linhas do catálogo apontam `<img>` para os seis endereços do Supabase, em rodízio. Com o bucket já público (T003), os endereços respondem `200 image/jpeg` — CA-10 fechado.

> **T003 ficou bloqueada durante a execução e foi destravada depois.** Na primeira verificação o bucket ainda era privado: `GET .../object/public/images/public/IMG_7382.JPG` devolvia `400 {"error":"Bucket not found"}` enquanto o endereço assinado do mesmo arquivo respondia `200` — o bucket existia e o arquivo estava lá, faltava a política pública. Depois de o responsável marcar o bucket como público no painel, os seis endereços passaram a responder `200`, e a fase foi fechada sem precisar refazer nada: a semeadura já gravava os endereços certos.
...
>
> ⚠️ **Corrigido depois da T032, e o defeito era desta tarefa.** A primeira versão apenas *omitia* a variável no ramo sem credencial. Como o processo filho roda com `ASPNETCORE_ENVIRONMENT=Development`, o ASP.NET Core carrega os *user secrets* da máquina — e assim que a chave real foi configurada lá para permitir a T032, o teste de CA-09 passou a cadastrar produto de verdade em vez de recusar. **Omitir não é negar.** Agora o ramo zera `SupabaseSettings__ChaveDeServico` explicitamente, e o teste passa mesmo com a chave presente na máquina, que é o caso difícil.
...
- [x] **T032** — Feita com a credencial real, e **encontrou um defeito que nenhum teste da suíte pegaria**. Cadastro manual pelo formulário: o arquivo subiu, o produto foi gravado, o redirecionamento aconteceu, o endereço guardado é um `Guid` sem vestígio do nome original (CA-07) e a imagem responde `200 image/png` no endereço público (CA-06). A categoria `Externo` passa.
>
> ⚠️ **O defeito:** o formato novo de chave do Supabase (`sb_secret_…`) **não é um JWT**, e o Storage recusa com `403 "Invalid Compact JWS"` quando só o cabeçalho `Authorization: Bearer` é enviado. O adaptador foi escrito seguindo `FreteServiceMelhorEnvio`, que manda só `Authorization` — e por isso todo cadastro real falhava com "Não foi possível enviar a imagem agora.", enquanto os testes de unidade (que afirmam a requisição, não a resposta de um servidor real) seguiam verdes. Corrigido acrescentando `apikey` com a mesma chave, e a asserção correspondente entrou em `ArmazenamentoSupabaseTests`. **É o argumento vivo a favor de a T032 existir:** ela é a única tarefa da spec que fala com o serviço de verdade.
