...
> **Ordem executada:** `002` → `003` → `001` → `004` → `005` → `006`. A `001`
> originalmente esperava a `004`/`005` para resolver papéis, mas a pendência
> foi resolvida com o mínimo viável embutido nela própria (papel
...
>
> **Próxima:** `007`, que valida tudo pela interface via Playwright. Ficou
> atrás da `006` de propósito — não faz sentido gravar E2E que confirmem uma
> mensagem de erro que a auditoria já sabia estar errada.
